...
class Player(object):
  ...
  def gamePlan(self):
    if self.strategy == "Aggres":
        max_rolls = 10
        self.round_score = 0
        self.round_rolls = 0 
        #Rolls anywhere between 1 to 10 times
        ...
        if self.score < 75:
            ...
            self.round_score = 0
            self.round_rolls = 0
            #Rolls anywhere betwen 1 to 5 times
            for i in range(random.randint(1,max_rolls)):
                self.round_rolls += 1
                roll = self.dice.Roll()
                if roll == 0:
            ...
            self.round_score = 0
            self.round_rolls = 0
            #While score is at least 75 roll until skunk out or win
        ...
        self.round_score = 0
        self.round_rolls = 0
        #Rolls anywhere between 1 to 4 times
        for i in range(random.randint(1,max_rolls)):
            self.round_rolls += 1
            roll = self.dice.Roll()
            if roll == 0:
...
print("Players", "Strategy", "Win %")
for winner,wins, in Winners.items():
    print(winner.name, '   ', winner.strategy, ' ', len(wins)/runs)
```
